...
class Recall(Metrics):
    ...
    def __init__(self):
        self.n = 0
        self.threshold = [0.4, 0.4, 0.4, 0.4]
        self.true_positives = [0] * 5
        self.false_positives = [0] * 5
        self.true_negatives = [0] * 5
        self.false_negatives = [0] * 5
            ...
            for j in range(n_class):
                if predict_label[i][j] == label[i][j].cpu():
                    if predict_label[i][j] == 1:
                        ...
                        self.true_positives[4] += 1
                    else:
                        self.true_negatives[j] += 1
                        self.true_negatives[4] += 1
                else:
                    if predict_label[i][j] == 1:
                        self.false_positives[j] += 1
                        self.false_positives[4] += 1
                    else:
                        self.false_negatives[j] += 1
                        self.false_negatives[4] += 1

    def get_f1(self):
        recall = self.true_positives[4] / (self.true_positives[4] + self.false_negatives[4] + 1e-20)
        precision = self.true_positives[4] / (self.true_positives[4] + self.false_positives[4] + 1e-20)
